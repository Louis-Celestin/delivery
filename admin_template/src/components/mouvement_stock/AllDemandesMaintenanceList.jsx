...
export default function AllDemandesMaintenanceList() {
    ...
    useEffect( ()=>{
        const fetchDemandeForms = async () =>{
            ...
            try{
                ...
                const demandesMaintenance = response.filter(item => {
                    return  item.role_id_demandeur === 6
                });
                setDemandeForms(demandesMaintenance)
    ...
    const titleTemplate = (demandeForms) =>{
        let title = '';
        let linkSee = `/demande-vue-details/${demandeForms.id_demande}`;
