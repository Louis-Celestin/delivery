...
export default function ModifyDemandeSupportInputs() {
  ...
  useEffect( ()=>{
    const fetchDemandeInfos = async () => {
      ...
      try{
        ...
        setProduitsDemandesTable(parsedProduitsDemandes);
        setQteProduits(parsedProduitsDemandes.length)
        switch (data.type_demande_id) {
        case 1: 
          setTypeDemande("CHARGEUR DECOMMISSIONNE"); 
          setDemandeID(1);
          break;
        default: setTypeDemande("Inconnu"); break;
      }
      }catch(error){
  ...
  const handleConfirm = () => {
    ...
    const isDuplicate = produitsDemandes.some(prod => prod.serialNumber === terminalSN);
    if (isDuplicate) {
  ...
  const handleDemande = async (e) => {
    ...
    if(!demandeID){
        Swal.fire({
        title: "Error",
        text: "Vous devez choisir un type de demande",
        icon: "error"
        });
        return;
    }
    ...
    try{
    // const response = await productDeliveries.deliver(commentaire, type_livraison_id, user_id, isAncienne, produitsDemandes)
    const response = await demandes.updateDemande(id, produitsDemandes, commentaire, statut_demande, type_demande_id, user_id)
