  ...
  useEffect( ()=>{
    const fetchLivraisons = async () =>{
      try{
        ...
        // @ts-ignore
        .filter(item => {
          return item.statut_livraison === 'en_cours' && item.type_livraison_id === 8
        })
        // @ts-ignore
        .map(item => ({
          ...item,
      ...
      >
        ...
        <ul className="flex flex-col h-auto overflow-y-auto custom-scrollbar">
          ...
          {livraisonEnAttente.map(livraison =>{
            ...
            let livraisonTitle = ''
            // @ts-ignore
            if (livraison.type_livraison_id === 8) {
              livraisonTitle = 'Livraison CHARGEUR (DECOM RI NOK)';
              // @ts-ignore
          }
            return(
              <li>
                <DropdownItem
                  onItemClick={() =>{
                    closeDropdown
                    // @ts-ignore
                    navigate(`/formulaire-support-chargeur-recu/${livraison.id_livraison}`)
                  }}
        ...
        <Link
          to="/toutes-les-livraisons-support"
