...
export default function AllSupportDeliveriesList({ filterType }) {
    ...
    const typeOptions = [
        ...
        {label: 'Chargeur (TPE DECOM RI NOK)', value:8},
    ]
    ...
    useEffect( ()=>{
        const fetchDeliveryForms = async () =>{
            ...
            try{
                ...
                const livraisonsSupport = response.filter(item => {
                    return  item.type_livraison_id === 8
                });
                setDeliveryForms(livraisonsSupport)
    ...
    const titleTemplate = (deliveryForms) =>{
        ...
        if (deliveryForms.type_livraison_id === 1) {
            ...
            title = 'Livraison CHARGEUR (TPE DECOM RI NOK)'
            link = `/formulaire-support-chargeur-recu/${deliveryForms.id_livraison}`
        }
        return (
    ...
    const actionTemplate = (deliveryForms) =>{
        ...
        if (deliveryForms.type_livraison_id === 8) {
            linkSee = `/formulaire-support-chargeur-recu/${deliveryForms.id_livraison}`
            // linkModify = `/form-modify-livraison-chargeur/${deliveryForms.id_livraison}`
        }
        return(
